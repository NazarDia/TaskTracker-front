...
import { fetchBoards, getBoardByID, deleteBoard } from '../../redux/boards/operations';
      ...
      <div className={s.container}>
        ...
        {boards.length > 0 ? (
          <ul className={s.boardList}>
            {boards.map((board) => (
              <li key={board._id} onClick={() => handleBoardClick(board)} className={`${s.boardItem} ${selectedBoard === board._id ? s.active : ''}`}>
                <svg className={`${s.boardIcon} ${selectedBoard === board._id ? s.checked : ''}`} width="18" height="18">
                  <use href={`${sprite}#${board.icon}`} />
                </svg>
                <span className={s.boardTitle}>{board.title}</span>
                {selectedBoard === board._id && (
                  <div className={s.iconButtons}>
                    <button onClick={() => handleEditBoard(board)} className={`${s.editButton} ${selectedBoard === board._id ? s.checked : ''}`}>
                      <svg width="16" height="16">
                        <use href={`${sprite}#pencil`} />
                      </svg>
                    </button>
                    <button onClick={() => handleDeleteBoard(board)} className={`${s.deleteButton} ${selectedBoard === board._id ? s.checked : ''}`}>
                      <svg width="16" height="16">
